...
module.exports = class FileController {
    ...
    upload(req, res) {
        ...
        this.fileService.upload(path, files).then(() => res.end()).catch(error => {
            console.error(error);
            res.status(500).end();
        });
    }
    ...
    delete(req, res) {
        ...
        this.fileService.delete(path).then(() => res.end()).catch(error => {
            console.error(error);
            res.status(500).end();
        });
    }
    ...
    rename(req, res) {
        ...
        this.fileService.rename(path, name, newName).then(() => res.end()).catch(error => {
            console.error(error);
            res.status(500).end();
        });
    }
